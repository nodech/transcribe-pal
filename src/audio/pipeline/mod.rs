...
pub use remixer::RemixerAvg;
pub use resampler::ResampleProcessor;
...
impl PipelineBuilder {
    ...
    #[instrument(level = "debug", name = "audio_pipeline.builder", skip_all)]
    pub fn with_stage<F, S, E>(
        mut self,
        init_stage: F,
    ) -> Result<Self, PipelineError>
    where
        S: AudioProcessor + Send + 'static,
        F: FnOnce(AudioSpec) -> Result<Option<S>, E>,
        E: std::error::Error + Sync + Send + 'static,
    {
        debug!(spec = ?self.current_spec, "adding new stage");
        let stage =
            init_stage(self.current_spec).map_err(PipelineError::new)?;
